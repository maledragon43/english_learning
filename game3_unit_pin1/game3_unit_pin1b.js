(function(){
  const { COLORS, palette, post, autoResize, tts, updateCharacterState } = window.g3;
  const grid = document.getElementById('grid');
  const banner = document.getElementById('statusBanner');

  const rows = 3, cols = 6;
  const TOTAL_PAIRS = 9; // 18 cards (9 pairs)

  function shuffle(a){ return a.map(v=>[Math.random(),v]).sort((x,y)=>x[0]-y[0]).map(x=>x[1]); }

  function buildDeck(){
    const colors = COLORS.slice(0, TOTAL_PAIRS);
    const deck = [];
    
    // Create pairs of cards
    colors.forEach((color, index) => {
      deck.push({ id: `card-${index}-a`, color: color, pairId: index });
      deck.push({ id: `card-${index}-b`, color: color, pairId: index });
    });
  ...
  function render(){
    ...
    deck.forEach(card => {
      ...
      cardEl.setAttribute('data-pair-id', card.pairId);
       ...
       front.style.background = palette[card.color];
      
             const back = document.createElement('div');
       back.className = 'card-face card-back';
       const backImg = document.createElement('img');
       backImg.src = '../assets/images/backside.png';
